...
def main():
    ...
    category = [(id, category['name']) for id, category in enumerate(categories, 1)]
    grades = ('a', 'b', 'c', 'd', 'e')
    ...
    for idx in range(len(categories)):
        products = get_category_products(categories, idx + 1)
        #category = [(id, category['name']) for id, category in enumerate(categories, 1)]
        product = [(product['code'],
				  ...
				  for product in products if 'nutrition_grade_fr' in product.keys() and 
				  								  product['code'] and 
				  								  product['product_name_fr'] and 
				  								  product['ingredients_text_fr'] and 
												  product['stores'] and
                                                  product['nutrition_grade_fr'] != '']
        with conn:
